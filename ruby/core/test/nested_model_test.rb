...
class NestedModelTest < MiniTest::Unit::TestCase
  ...
  def test_find_variable_in_child()
    assert_equal A_VALUE_CHILD, @model.get_variable_value(A_NAME)
  end

  def test_find_variable_in_parent()
    assert_equal B_VALUE_PARENT, @model.get_variable_value(B_NAME)
  end

  def test_return_not_found_when_variable_not_found()
    assert_equal Mullet::Model::NOT_FOUND, @model.get_variable_value(:c)
  end
end
